 ...
 * 
 * This class is an AVL Tree that does not allow the insertion of duplicate numbers
 */
class Set {
private:
	Node* root;
	 ...
	 * 
	 * Calculates the height of a node in a tree in cases where the height cannot be given by the attribute
	 * 
	 ...
	 *
	 * This operation is typically used to maintain the balance of a binary search tree,
	 * such as an AVL tree, when the right subtree of a node becomes taller than the left
	 *
	 ...
	 *
	 * This operation is typically used to maintain the balance of a binary search tree,
	 * such as an AVL tree, when the left subtree of a node becomes taller than the right
	 *
	 * @param p Pointer to the root of the subtree to rotate
	 * @return Node* Pointer to the new root of the rotated subtree
	 */
	 ...
	 * @param key The key that was recently inserted, used to determine the rotation type
	 ...
	 * 
	 * Inserts a new value into a binary tree, balancing the tree in the process recursively.
	 * 
	 * @param key The value to be inserted
	 * @param node The root node of the (sub)tree where the value is to be inserted
	...
	/**
	 * @brief Recursively clears (deletes) all nodes in the subtree rooted at the given node.
	 *
	 * This function performs a post-order traversal, deleting the left and right children
	 * of each node before deleting the node itself, ensuring that memory is properly freed.
	 *
	 ...
	 *
	 * This function recursively traverses the tree to search for a node with the specified
	 * key, returning `true` if found and `false` otherwise.
	 *
...
public:
	 ...
	 * 
	 * Updates the root node with the new tree generated after the recursive insertion
	 * 
	 * @param key The value to be inserted
	 */
	void insert(int key);
	...
	/**
	 * @brief Checks if the tree contains a node with the given key, starting from the root.
	 *
	 * This function is a wrapper around the recursive `contains` function, starting the
	 * search from the root node of the tree.
	 *
	 * @param key The key to search for in the tree.	
	 * @return bool `true` if the key is found, `false` otherwise.
	 */
	 ...
	 *
	 * This function is a wrapper around the recursive `getHeight` function, calculating
	 * the height of the tree by starting from the root node.
	 *
	 * @return int The height of the tree.
	 */
	int getHeight() const;
	 ...
	 *
	 * This function swaps the root nodes and sizes of the current set and the given set `t`.
	 * After the swap, both sets will contain the data and size of the other.
	 *
	 ...
	 *
	 * This function returns `true` if the tree has no nodes (i.e., size is zero and root is null),
	 * and `false` otherwise.
	 *
